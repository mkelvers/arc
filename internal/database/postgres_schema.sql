...
-- Anime identity is provider-neutral. Product tables still keep their existing
-- numeric anime_id columns during the compatibility migration, while this
-- registry records which provider identifiers refer to the same anime.
...
CREATE TABLE IF NOT EXISTS anime_external_id (
    anime_identity_id BIGINT NOT NULL REFERENCES anime_identity(id) ON DELETE CASCADE,
    provider TEXT NOT NULL CHECK(provider IN ('anilist', 'mal', 'tmdb_tv', 'tmdb_movie', 'allanime')),
...
WITH resolved_identity AS (
    SELECT
        anilist.external_id::BIGINT AS anilist_id,
        mal.external_id::BIGINT AS mal_id
    FROM anime_external_id AS anilist
    JOIN anime_external_id AS mal
        ON mal.anime_identity_id = anilist.anime_identity_id
        AND mal.provider = 'mal'
        AND mal.external_id ~ '^[0-9]+$'
    WHERE anilist.provider = 'anilist'
      AND anilist.external_id ~ '^[0-9]+$'
)
SELECT
    base.anilist_id,
    COALESCE(base.mal_id, identity.mal_id) AS mal_id,
    base.tmdb_media_type,
    base.tmdb_id,
    base.tmdb_season,
    FALSE AS canonical
FROM anime_external_mapping AS base
LEFT JOIN resolved_identity AS identity ON identity.anilist_id = base.anilist_id
WHERE NOT EXISTS (
    SELECT 1
...
SELECT
    inferred.anilist_id,
    COALESCE(inferred.mal_id, identity.mal_id) AS mal_id,
    inferred.tmdb_media_type,
    inferred.tmdb_id,
    inferred.tmdb_season,
    FALSE AS canonical
FROM anime_inferred_mapping AS inferred
LEFT JOIN resolved_identity AS identity ON identity.anilist_id = inferred.anilist_id
WHERE NOT EXISTS (
    SELECT 1
    FROM anime_mapping_override AS override
...
SELECT
    override.anilist_id,
    COALESCE(override.mal_id, identity.mal_id) AS mal_id,
    override.tmdb_media_type,
    override.tmdb_id,
    override.tmdb_season,
    override.canonical
FROM anime_mapping_override AS override
LEFT JOIN resolved_identity AS identity ON identity.anilist_id = override.anilist_id
...
CREATE INDEX IF NOT EXISTS idx_anime_external_id_identity ON anime_external_id(anime_identity_id);
