...
| `src/graphql-error.ts` | GraphQL error contract | `@arc/shared/graphql/error` | Moved |
...
| `src/utils/array.ts` | Generic nullable-array utility | Inline at catalog call sites | Removed |
...
GraphQL transport errors belong beside the GraphQL documents and generated output in `@arc/shared`. Core does not own provider transport infrastructure.
