...
import { catalogRefreshKey, refreshCatalogPage } from '@arc/core/catalog/storage';
...
export async function refreshHomepage(season: MediaSeason, seasonYear: number) {
    ...
    await refreshCatalogPage(catalogRefreshKey(filters, 1), result.anime, result.hasNextPage);
    return result;
}
