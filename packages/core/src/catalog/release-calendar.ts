...
export function createReleaseCalendarOperations(source: CatalogSource) {
    async function releaseCalendar(now = new Date()) {
        ...
        const [rows, heartbeat] = await Promise.all([
            db
                ...
                .where(
                    and(
                        gte(animeAiringSchedule.airingAt, from),
                        lt(animeAiringSchedule.airingAt, to)
                    )
                )
    ...
    async function refreshReleaseCalendar(now = new Date()) {
        const { from, to } = releaseCalendarWindow(now);
        const entries = await source.releaseCalendar(from, to);
    ...
    return { refreshReleaseCalendar, releaseCalendar };
}
