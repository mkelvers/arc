...
>
    ...
    >
        ...
        <div class="mt-12 lg:mt-[clamp(3.75rem,3.9vw,5rem)]">
            <section
                id="anime-details"
                class={`grid min-w-0 grid-cols-1 gap-8 overflow-hidden text-sm leading-6 text-muted md:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] md:gap-12 lg:gap-[clamp(4rem,10.75vw,13.75rem)] lg:text-[clamp(0.9375rem,0.88vw,1.125rem)] lg:leading-[clamp(1.4rem,1.56vw,2rem)] ${detailsExpanded ? 'max-h-320' : 'max-h-24 mask-[linear-gradient(to_bottom,black_45%,transparent_100%)]'}`}
            >
