<script lang="ts">
    ...
    let loadedSelection = $state(
        untrack(() => browseSearchParams(data.filters).toString()),
    );
    let anime = $state<AnimeCardModel[]>(untrack(() => data.anime));
    let nextPage = $state<number | null>(
        untrack(() => (data.hasNextPage ? data.page + 1 : null)),
    );
    let loading = $state(false);
    let sentinel = $state<HTMLDivElement>();
    let activeRequest: AbortController | undefined;
    let categoryQuery = $state('');
    let categoryNeedle = $derived(
        categoryQuery.trim().toLocaleLowerCase('en'),
    );
    ...
    let visibleTags = $derived(
        data.taxonomy.tags.filter((value) =>
            value.toLocaleLowerCase('en').includes(categoryNeedle),
        ),
    );
    ...
    async function loadMore() {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    `Browse page request returned ${response.status}`,
                );
            }
            ...
            if (!result) {
                throw new TypeError(
                    'Browse page request returned an invalid response',
                );
            }
            ...
            anime = [
                ...anime,
                ...result.anime.filter(({ id }) => !existing.has(id)),
            ];
        ...
        if (
            ...
            loadedSelection === requestSelection
        ) {
...
<main class="min-h-dvh bg-canvas px-5 py-10 text-foreground sm:px-10 sm:py-12 lg:px-16 lg:py-16">
    <section class="mx-auto w-full max-w-[96rem]" aria-labelledby="browse-title">
        <h1 id="browse-title" class="mb-6 text-2xl font-semibold">
            Browse Anime
        </h1>

        <div class="mb-10 flex flex-wrap items-stretch border-y border-border">
            <label class="flex h-12 min-w-64 flex-1 items-center gap-3 px-3 text-muted focus-within:text-foreground">
                <MagnifyingGlassIcon
                    size="1.2rem"
                    weight="regular"
                    class="shrink-0"
            ...
            >
                ...
                {#snippet content()}
                    <div role="menu" aria-label="Genre and tag filters">
                        <div
                            class="sticky top-0 z-10 border-b border-border bg-panel p-2"
                        >
                            <label class="flex h-9 items-center gap-2 px-2 text-muted focus-within:text-foreground">
                                <MagnifyingGlassIcon
                                    size="1rem"
                                    weight="regular"
                                    aria-hidden="true"
                        ...
                        <a
                            ...
                            class:text-muted={Boolean(
                                data.filters.genre || data.filters.tag,
                            )}
                        ...
                        {#if visibleGenres.length}
                            <p class="border-t border-border px-5 pt-3 pb-1 text-xs font-medium tracking-wide text-subtle uppercase">
                                Genres
                            </p>
                        ...
                        {#if visibleTags.length}
                            ...
                            {#each visibleTags as tag}
                                <a
                                    ...
                                    aria-current={data.filters.tag === tag
                                        ? 'page'
                                        : undefined}
            ...
            <Tooltip text={data.filters.order === 'desc' ? 'Descending' : 'Ascending'}>
                <button
                    ...
                    onclick={() =>
                        selectFilter({
                            order:
                                data.filters.order === 'desc' ? 'asc' : 'desc',
                        })}
                >
        ...
        {#if anime.length}
            <div class="grid grid-cols-2 gap-x-2 gap-y-8 sm:grid-cols-3 sm:gap-x-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7">
