<script lang="ts">
    ...
    let loadedSelection = $state(
        untrack(() => `${data.season}:${data.year}`),
    );
    let anime = $state<AnimeCardModel[]>(
        untrack(() => data.page.anime),
    );
    ...
    async function loadMore() {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    `Simulcast page request returned ${response.status}`,
                );
            }
            ...
            if (!result) {
                throw new TypeError(
                    'Simulcast page request returned an invalid response',
                );
            }
...
<svelte:head>
    ...
    <meta
        name="description"
        content={`Browse anime from the ${data.label} simulcast season.`}
    />
</svelte:head>

<main class="min-h-dvh bg-canvas px-5 py-10 text-foreground sm:px-10 sm:py-12 lg:px-16 lg:py-16">
    <section
        class="mx-auto w-full max-w-[96rem]"
        aria-labelledby="simulcast-title"
    >
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <h1 id="simulcast-title" class="text-2xl font-semibold">
                Simulcast Season
            </h1>
            <Dropdown
                id="simulcast-season"
        ...
        {#if anime.length}
            <div class="grid grid-cols-2 gap-x-2 gap-y-8 sm:grid-cols-3 sm:gap-x-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        ...
        {#if nextPage !== null}
            ...
            >
                ...
                {:else}
                    <span class="sr-only">
                        More releases load automatically while scrolling.
                    </span>
                {/if}
            </div>
        {/if}
