  ...
  {#if !data.artwork}
    <section class="max-w-2xl border border-border bg-surface p-6 sm:p-8">
      ...
      <p class="mt-3 leading-7 text-muted">
        This release is still available on Arc, but no reliable TMDB match was found. Its media
        options will appear automatically when a confident match becomes available.
      </p>
      <a
        ...
        {#each data.artwork.logos as image}
          <form method="POST">
            ...
            >
              ...
              <span class="sr-only"
                >{image.width} × {image.height}, {image.language ?? 'no language'}</span
              >
            </button>
          </form>
        {/each}
