<script lang="ts">
  ...
  $effect(() => {
    ...
    const timeout = setTimeout(() => {
      void goto(next ? `/search?q=${encodeURIComponent(next)}` : '/search', {
        replaceState: true,
        keepFocus: true,
        noScroll: true,
      });
  ...
  <div class="mx-auto w-full max-w-6xl px-5 py-8 sm:px-10 sm:py-10 lg:px-0 lg:py-12">
    {#if data.query}
      <section aria-labelledby="search-results-title">
        <h1 id="search-results-title" class="mb-5 text-lg font-semibold">Top results</h1>

        {#if data.results.length}
          <div
            class="grid grid-cols-2 gap-x-3 gap-y-8 sm:grid-cols-3 sm:gap-x-5 md:grid-cols-4 lg:grid-cols-5"
          >
            {#each data.results as result (result.id)}
              <AnimeCard anime={result} onselect={(anime) => recent.remember(anime)} />
            {/each}
          </div>
        {:else}
    ...
    {:else if recent.results.length}
      <section aria-labelledby="recent-results-title">
        <div class="mb-4 flex items-center justify-between gap-6">
          <h1 id="recent-results-title" class="text-lg font-semibold">Recent Search Results</h1>
          <button
            type="button"
        ...
        <ul class="flex flex-wrap gap-2">
          {#each recent.results as result (result.id)}
            <li
              class="flex min-w-0 max-w-full items-stretch bg-player-accent/30 text-xs font-semibold uppercase"
            >
              <a
                href={result.href}
