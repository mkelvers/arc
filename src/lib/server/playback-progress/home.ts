...
export async function getContinueWatchingCards(
    ...
    const progressEntries = await getRecentPlaybackProgress(
        userId,
        limit * 2,
    );
    ...
    const anilistIds = [
        ...new Set(progressEntries.map(({ anilistId }) => anilistId)),
    ];
    ...
    const cards = await Promise.all(
        progressEntries.map(async (progress) => {
            const episodes =
                episodesByAnime.get(progress.anilistId) ?? [];
            const currentIndex = episodes.findIndex(
                ({ episodeId }) => episodeId === progress.episodeId,
            );
            const current =
                currentIndex >= 0 ? episodes[currentIndex] : null;
            const target = progress.completed
                ...
                : (current ?? {
                      ...
                      audio: [],
                      imageUrl: null,
                      runtimeMinutes: Math.ceil(
                          progress.durationSeconds / 60,
                      ),
                      airDate: null,
                      overview: null,
            ...
            const storedMedia = await anime.tmdb
                .getStoredMedia(progress.anilistId)
                .catch((cause) => {
                    console.error(
                        `Stored TMDB media failed for AniList ${progress.anilistId}`,
                        cause,
                    );
                    return null;
                });
            const details = progress.details
                ? toAnimeDetails(progress.details)
                : null;
            const backdrop =
            ...
            const continuingCurrent =
                !progress.completed &&
                target.episodeId === progress.episodeId;
            const runtimeMinutes =
                target.runtimeMinutes ??
                (continuingCurrent
                    ? Math.ceil(progress.durationSeconds / 60)
                    : null);
