...
function seasonEvidenceScore(anime: AniListAnime, episodes: SpecialEpisodeEvidence[]) {
  ...
  const matchingTitles = relevant.filter(({ name }) => {
    ...
    return qualifiers.some((qualifier) => title.includes(qualifier) || qualifier.includes(title));
  }).length;
...
export function specialEpisodeEvidenceScore(
  ...
  return Math.max(0, ...[...seasons.values()].map((season) => seasonEvidenceScore(anime, season)));
}
