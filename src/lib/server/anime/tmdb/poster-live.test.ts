...
const animeIds = [
    16498, 18397, 21459, 97940, 99263, 101280, 101922, 105310, 105333, 108465,
    113415, 11757, 120377, 125206, 131573, 137822, 146984, 151807, 153288,
    154587,
];
...
async function tmdbPosters(seriesId: number) {
    ...
    return (data.posters ?? []).filter(
        ({ aspect_ratio, file_path, height, width }) =>
            ...
            height &&
            aspect_ratio &&
            Math.abs(aspect_ratio - 2 / 3) < 0.08,
    );
}
...
describe('live anime card artwork providers', () => {
    liveTest(
        ...
        async () => {
            ...
            expect(
                new Set(media.map(({ coverImage }) => coverImage?.extraLarge))
                    .size,
            ).toBe(animeIds.length);
        },
        20_000,
    );
    ...
    liveTest(
        ...
        async () => {
            const samples = await Promise.all(
                [95479, 1429, 65930, 82684].map(tmdbPosters),
            );
    ...
    liveTest(
        ...
        async () => {
            ...
            for (const id of expected) {
                expect(first.entries.some(({ malId }) => malId === id)).toBe(
                    true,
                );
                expect(final.entries.some(({ malId }) => malId === id)).toBe(
                    true,
                );
            }
        },
