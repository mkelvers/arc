...
function metadataScore(candidates: EpisodeCandidate[]) {
    return candidates.reduce(
        (score, episode) =>
            score +
            Number(
                Boolean(episodeTitleKey(episode.title)),
            ) +
...
export function releaseEpisodeGroup(
    ...
    const matches = blocks.flatMap((block) => {
        ...
        const dateMatch = Boolean(
            expectedStart && actualStart === expectedStart,
        );
            ...
            ? [
                  {
                      ...
                      identityScore:
                          ...
                          Number(expectedCount !== null) *
                              100 +
                          Number(seasonMatch) * 50 +
                          titles.matches * 200,
                      metadataScore:
                          metadataScore(candidates),
                  },
              ]
            : [];
    });
    ...
    if (
        !best ||
        (alternate &&
            alternate.identityScore ===
                best.identityScore &&
            alternate.key !== best.key)
    ) {
        return null;
    }
