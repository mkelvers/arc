...
function posterCandidates(
    ...
    return (posters ?? [])
        .map(posterCandidate)
        .filter(
            (candidate): candidate is PosterCandidate => candidate !== null,
        );
}
...
async function posterRows(anilistIds: number[]) {
    ...
    return db
        ...
        .innerJoin(
            animeExternalIdLink,
            eq(animeExternalIdLink.externalIdId, source.id),
        )
        .innerJoin(
            animeReleasePoster,
            eq(animeReleasePoster.animeId, animeExternalIdLink.animeId),
        )
        .where(
            and(
...
export async function getStoredPosters(anilistIds: number[]) {
    ...
    return new Map(
        rows.flatMap(({ anilistId, poster }) =>
            poster.filePath
                ? [[anilistId, imageUrl(poster.filePath, 'w780')] as const]
                : [],
        ),
    );
}
