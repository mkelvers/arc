...
import type { AllAnimeEpisode } from '$lib/server/anime/allanime';
...
interface TmdbEpisodeCandidate extends TmdbEpisodeMetadata {
...
function seasonScore(
    ...
    if (expectedCount > 0 && season.episode_count === expectedCount) score += 80;
    if (season.episode_count >= largestSourceNumber) score += 10;
    if (startDate && season.air_date === startDate) score += 80;
    else if (startYear && seasonYear === startYear) score += 30;
    else if (startYear && seasonYear) score -= Math.abs(startYear - seasonYear) * 8;

    return score;
}

function sequenceScore(
    sequence: TmdbEpisodeCandidate[],
    start: number,
    ...
    if (sequence.length - start < largestSourceNumber) return -Infinity;

    let score = baseScore + 20;
    const first = sequence[start];
    const firstYear = Number(first?.rawAirDate.slice(0, 4)) || null;

    if (startDate && first?.rawAirDate === startDate) score += 120;
    else if (startYear && firstYear === startYear) score += 35;
    if (expectedCount > 0 && sequence.length - start === expectedCount) score += 30;
    ...
    for (const episode of source.slice(0, 4)) {
        ...
        if (media) score += titleMatchScore(episode.title, media.title);
    }
...
function candidateStarts(
    ...
    sequence.forEach((episode, index) => {
        if (startDate && episode.rawAirDate === startDate) starts.add(index);
        if (source[0]?.title && titleMatchScore(source[0].title, episode.title) >= 25) {
            starts.add(index);
        }
    });
...
async function getEpisodeMetadata(
    anime: AniListAnime,
    source: AllAnimeEpisode[],
): Promise<Map<string, TmdbEpisodeMetadata>> {
    const match = await resolveStored(anime);

    if (match.mediaType !== 'tv' || !source.length) return new Map();
    ...
    if (!series) throw new Error('TMDB series request failed', { cause: error });
    ...
    const largestSeason = Math.max(0, ...seasons.map(({ episode_count }) => episode_count));
    ...
    const fetched = await Promise.all(
        selected.map(async ({ season, score }) => {
            ...
            if (!response.data) return { episodes: [], score };
    ...
    let best:
        | { sequence: TmdbEpisodeCandidate[]; start: number; score: number }
        | undefined;

    for (const candidate of sequences) {
        for (const start of candidateStarts(candidate.episodes, source, startDate)) {
            ...
            if (!best || score > best.score) {
                best = { sequence: candidate.episodes, start, score };
            }
        }
    }

    if (!best || best.score < 60) return new Map();

    return new Map(
        source.flatMap((episode) => {
            if (!Number.isInteger(episode.number) || episode.number < 1) return [];
            const media = best.sequence[best.start + episode.number - 1];
