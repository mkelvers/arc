...
async function withSelections(
    ...
    const [preference] = await db
        ...
        .where(
            eq(
                animeArtworkPreference.externalIdId,
                match.externalIdId,
            ),
        )
        .limit(1);
    ...
    return {
        ...
        selectedBackdrop:
            artwork.backdrops.find(
                ({ filePath }) =>
                    filePath === preference?.backdropFilePath,
            ) ??
            artwork.backdrops[0] ??
            null,
        selectedLogo: logoHidden
            ...
            : (artwork.logos.find(
                  ({ filePath }) =>
                      filePath === preference?.logoFilePath,
              ) ??
              artwork.logos[0] ??
              null),
        selectedPoster: null,
            ...
            await tx
                ...
                .onConflictDoUpdate({
                    ...
                    set: {
                        ...
                        height: sql.raw(
                            `excluded.${animeArtwork.height.name}`,
                        ),
                        language: sql.raw(
                        ...
                        width: sql.raw(
                            `excluded.${animeArtwork.width.name}`,
                        ),
                    },
                });
        }
