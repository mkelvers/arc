...
describe('TMDB episode identity matching', () => {
  test('maps a TV finale special instead of leaking into the next season', () => {
    const episodes = source([
      ...Array.from(
        { length: 12 },
        (_, index) =>
          [String(index + 1), index + 1, `Regular ${index + 1}`] as [string, number, string]
      ),
      ['13', 13, 'Winter Days!'],
    ]);
  ...
  test('uses chronology when a provider repeats the preceding title', () => {
    const metadata = matchEpisodeMetadata(
      ...
      [
        ...
        candidate(0, 4, "Extra: Rimuru's Glamorous Life as a Teacher, Part 1", '2020-03-27'),
    ...
    const metadata = matchEpisodeMetadata(
      ...
      source(
        Array.from(
          { length: 4 },
          (_, index) =>
            [String(index + 1), index + 1, `Episode ${index + 1}`] as [string, number, string]
        )
      ),
  ...
  test('rejects exact titles from an earlier cour', () => {
    ...
    const metadata = matchEpisodeMetadata(
      ...
      source(
        staleTitles.map(
          (title, index) => [String(index + 1), index + 1, title] as [string, number, string]
        )
      ),
      [
    ...
    expect(staleTitles.map((_, index) => metadata.get(String(index + 1))?.episodeNumber)).toEqual([
