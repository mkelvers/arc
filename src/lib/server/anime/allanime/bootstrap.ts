    ...
    ([expression]) => {
      ...
      const part = calls
        .map((call) => {
          ...
          return resolver ? valueAt(resolver, callArguments(call[2], call[3])) : undefined;
        })
        .join('');
...
export function decodeChunk(chunk: string) {
  ...
  const strings = [...table[2].matchAll(/"((?:\\.|[^"\\])*)"|'((?:\\.|[^'\\])*)'/g)].map((match) =>
  ...
  for (const client of chunk.matchAll(clientPattern)) {
    ...
    for (const array of arrays) {
      ...
      for (let index = 0; index < seed.length; index++) {
        seed[index] = buildId.charCodeAt(index % buildId.length) ^ ((index * 17 + 31) & 0xff);
      }
