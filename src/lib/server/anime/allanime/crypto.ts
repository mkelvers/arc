...
export async function getCrypto(refresh = false) {
  ...
  if (!bootstrap) {
    ...
    const secret = createHmac('sha256', client.mask).update(`aa-boot:${client.buildId}`).digest();
    let status = 0;
...
export function lease(crypto: StreamCrypto, queryHash: string) {
  ...
  const iv = createHash('sha256')
    .update(`${crypto.epoch}:${crypto.buildId}:${queryHash}:${timestamp}:${contentLane}`)
    .digest()
    .subarray(0, 12);
...
export function decrypt(value: string, key: Buffer) {
  ...
  return JSON.parse(
    Buffer.concat([decipher.update(encrypted.subarray(13, -16)), decipher.final()]).toString('utf8')
  ) as unknown;
}
