...
function exactIdentity(
    ...
    return (
        identity.anilistId === anime.id || Boolean(anime.idMal && identity.malId === anime.idMal)
    );
}
...
async function findAnimeId(anime: Parameters<PlaybackProvider['getEpisodes']>[0], refresh = false) {
    ...
    for (const title of titles) {
        ...
        const candidates = searchCandidates(
            ...
            (candidate) =>
                normalized.has(normalizedProviderTitle(candidate.title)) &&
...
async function getStreams(
    ...
    const payload = record(
        await requestJson(`/api/frontend/episode/${match.providerId}/languages`)
    );
