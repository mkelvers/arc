...
function requestHeaders(accept: string) {
  return {
    ...
    ...(env.ANIDB_COOKIE ? { Cookie: env.ANIDB_COOKIE.trim() } : {}),
  };
}
...
function searchCandidates(html: string) {
  const $ = load(html);
  const candidates = new Map<number, { id: number; title: string; path: string }>();
  ...
  $('a[href]').each((_, element) => {
    ...
    if (url.origin !== baseUrl || !Number.isSafeInteger(id) || id <= 0) {
      return;
    }
...
function pageIdentity(html: string) {
  ...
  $('a[href]').each((_, element) => {
    const href = $(element).attr('href') ?? '';
    const anilist = href.match(/^https:\/\/anilist\.co\/anime\/(\d+)/);
    const mal = href.match(/^https:\/\/myanimelist\.net\/anime\/(\d+)/);
...
function exactIdentity(
  ...
  if (identity.anilistId !== null && identity.anilistId !== anime.id) {
    return false;
  }
  if (identity.malId !== null && anime.idMal && identity.malId !== anime.idMal) {
    return false;
  }

  return identity.anilistId === anime.id || Boolean(anime.idMal && identity.malId === anime.idMal);
}

async function findAnimeId(anime: Parameters<PlaybackProvider['getEpisodes']>[0], refresh = false) {
  if (!refresh) {
  ...
  for (const title of titles) {
    ...
    const candidates = searchCandidates(
      await requestText(`${search.pathname}${search.search}`, 'text/html')
    ).filter(
      (candidate) =>
        normalized.has(normalizedProviderTitle(candidate.title)) && !visited.has(candidate.id)
    );

    for (const candidate of candidates.slice(0, 6)) {
      visited.add(candidate.id);
      const identity = pageIdentity(await requestText(candidate.path, 'text/html'));
  ...
  throw new Error(`AniDB has no exact identity match for AniList ${anime.id}`);
}

async function loadEpisodes(animeId: number) {
  const payload = record(await requestJson(`/api/frontend/anime/${animeId}/episodes`));
...
async function providerEpisodes(anime: Parameters<PlaybackProvider['getEpisodes']>[0]) {
  ...
  try {
    ...
    if (!(cause instanceof AniDbRequestError) || cause.status !== 404) {
      throw cause;
    }
  ...
  if (!episodes.length) {
    throw new Error(`AniDB returned an empty episode inventory for AniList ${anime.id}`);
  }
...
function hlsStream(html: string): ProviderStream {
  ...
  const script = $('script')
    ...
    .join('\n')
    .replaceAll('\\/', '/');
  const match = script.match(/https:\/\/hls\.anidb\.app\/[^"'\\\s<]+?\.m3u8(?:\?[^"'\\\s<]*)?/i);
...
async function getStreams(
  ...
  if (!match) {
    throw new Error(`AniDB has no episode ${episode.number} for AniList ${anime.id}`);
  }

  const payload = record(await requestJson(`/api/frontend/episode/${match.providerId}/languages`));
  ...
  const languages = payload.languages.flatMap((value) => {
    ...
    if (!mode || !requested.has(mode) || typeof embed !== 'string') {
      return [];
    }
