...
describe('AniNeko provider', () => {
    test('matches a title with a (TV) disambiguator and resolves streams', async () => {
        globalThis.fetch = mock(async (input: string | URL | Request) => {
            ...
            if (
                url.hostname === 'anineko.to' &&
                url.pathname === '/ajax/search'
            ) {
                return response(searchPayload());
            }
            if (
                url.hostname === 'anineko.to' &&
                url.pathname === '/watch/jujutsu-kaisen-tv'
            ) {
                return response(watchPage());
            }
            if (
                url.hostname === 'anineko.to' &&
                url.pathname === '/watch/jujutsu-kaisen-tv/ep-1'
            ) {
        ...
        const streams = await aninekoProvider.getStreams(
            anime,
            { id: '1', number: 1 },
            ['sub', 'dub'],
        );
