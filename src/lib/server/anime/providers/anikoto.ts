...
async function specialReleaseEpisode(
  ...
  for (const result of results) {
    ...
    for (const candidate of result.value) {
      if (
        standaloneSpecialMatches(anime, episode, [candidate.title, candidate.alternativeTitle]) ||
        specialCollectionMatches(anime, episode, [candidate.title, candidate.alternativeTitle])
      ) {
        candidates.set(candidate.id, candidate);
      }
    }
  ...
  for (let offset = 0; offset < values.length; offset += 12) {
    ...
    for (const result of batch) {
      ...
      const playable = result.value.episodes.filter((candidate) => episodeModes(candidate).length);
...
async function getStreams(
  ...
  const requested = [...new Set(modes)].filter(
    (mode): mode is 'sub' | 'dub' => mode !== 'raw' && Boolean(validEmbed(match.embeds[mode], mode))
  );
