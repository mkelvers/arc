...
async function findAnimeId(anime: ProviderAnime, refresh = false) {
  ...
  for (const title of titles) {
    const candidates = searchResults(
      ...
      (candidate) =>
        exactTitles.has(normalizedProviderTitle(candidate.name)) && !visited.has(candidate.id)
    );
...
async function resolveStream(id: string, mode: AudioMode) {
  ...
  const payload = record(
    await requestJson(new URL(`/stream/getSources?id=${sourceId}`, megaplayUrl), page.toString())
  );
