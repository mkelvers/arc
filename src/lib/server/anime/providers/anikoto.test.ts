  ...
  test('resolves independent sub and dub HLS sources with per-mode captions', async () => {
    ...
    globalThis.fetch = mock(async (input: string | URL | Request) => {
      ...
      if (url.hostname === 'megaplay.buzz' && url.pathname.startsWith('/stream/s-2/')) {
        ...
        return response(`<title>File ${mode === 'dub' ? '13454' : '13461'} - MegaPlay</title>`);
      }
    ...
    const streams = await anikotoProvider.getStreams(anime, { id: '1', number: 1 }, ['sub', 'dub']);
  ...
  liveTest(
    ...
    async () => {
      ...
      const first = episodes.find(
        (episode) =>
          episode.number === 1 && episode.audio.includes('sub') && episode.audio.includes('dub')
      );
