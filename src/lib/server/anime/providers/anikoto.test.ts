...
describe('AniKoto provider', () => {
    ...
    test('finds a fractional special stored as a standalone provider release', async () => {
        ...
        globalThis.fetch = mock(async (input: string | URL | Request) => {
            ...
            if (
                url.hostname === 'megaplay.buzz' &&
                url.pathname.startsWith('/stream/s-2/54553/')
            ) {
                return response(
            ...
            if (
                ...
                const mode =
                    url.searchParams.get('id') === '175614'
                        ? 'dub'
                        : 'sub';
                return response({
                    sources: {
        ...
        expect(streams.sub?.[0]?.url).toBe(
            'https://megap.kotocdn.site/hinata/sub/master.m3u8',
        );
        expect(streams.dub?.[0]?.url).toBe(
            'https://megap.kotocdn.site/hinata/dub/master.m3u8',
        );
    ...
    test('uses the sole provider episode for an exact one-episode movie', async () => {
        ...
        globalThis.fetch = mock(async (input: string | URL | Request) => {
            ...
            if (
                url.hostname === 'megaplay.buzz' &&
                url.pathname === '/stream/s-2/145439/sub'
            ) {
                return response(
                    '<title>File 220001 - MegaPlay</title>',
                );
            }
            if (
                url.hostname === 'megaplay.buzz' &&
        ...
        expect(streams.sub?.[0]?.url).toBe(
            'https://megap.kotocdn.site/reawakening/master.m3u8',
        );
    });
    ...
    test('maps fractional episodes by position in a provider specials collection', async () => {
        ...
        globalThis.fetch = mock(async (input: string | URL | Request) => {
                    ...
                    data: {
                        anime: {
                            ...
                            title:
                                'That Time I Got Reincarnated as a Slime Season 03: Specials',
                            alternative:
                                'Tensei shitara Slime Datta Ken 3rd Season Specials',
                        },
                        episodes: [
                            {
                                number: 1,
            ...
            if (
                url.hostname === 'megaplay.buzz' &&
                url.pathname === '/stream/s-2/127080/sub'
            ) {
                return response(
                    '<title>File 230002 - MegaPlay</title>',
                );
            }
            if (
                url.hostname === 'megaplay.buzz' &&
        ...
        expect(streams.sub?.[0]?.url).toBe(
            'https://megap.kotocdn.site/luminus/master.m3u8',
        );
