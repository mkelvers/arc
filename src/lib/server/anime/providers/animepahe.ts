...
function headers(accept: string) {
    return {
        ...
        ...(accept === 'application/json'
            ...
            : {}),
        ...(env.ANIMEPAHE_COOKIE
            ? { Cookie: env.ANIMEPAHE_COOKIE.trim() }
            : {}),
    };
}
...
async function requestText(url: URL, accept: string) {
    ...
    if (challenged) {
        throw new Error(
            'AnimePahe is blocked by a managed browser challenge',
        );
    }
    if (!response.ok) {
        throw new Error(
            `AnimePahe returned ${response.status} for ${url.pathname}`,
        );
    }
            ...
            ? [
                  {
                      ...
                      title: title.trim(),
                      session,
                      year: Number.isSafeInteger(year)
                          ? year
                          : null,
                  },
              ]
...
async function findAnime(anime: ProviderAnime, refresh = false) {
    ...
    throw new Error(
        `AnimePahe has no exact title match for AniList ${anime.id}`,
    );
}
...
async function loadEpisodes(reference: AnimePaheReference) {
    ...
    for (let page = 1; page <= 100; page++) {
        ...
        if (!payload || !Array.isArray(payload.data)) {
            throw new Error(
                'AnimePahe returned an invalid episode inventory',
            );
        }
...
async function providerEpisodes(anime: ProviderAnime) {
    ...
    if (!episodes.length) {
        throw new Error(
            `AnimePahe returned no episodes for AniList ${anime.id}`,
        );
    }
...
async function resolveKwik(value: string) {
    ...
    if (
        ...
        !(
            embed.hostname === 'kwik.cx' ||
            embed.hostname.endsWith('.kwik.cx')
        )
    ) {
...
async function getStreams(
    ...
    if (!match) {
        throw new Error(
            `AnimePahe has no episode ${episode.number} for AniList ${anime.id}`,
        );
    }

    const page = new URL(
        `/play/${reference.session}/${match.session}`,
        baseUrl,
    );
    ...
    $('[data-src]').each((_, element) => {
        ...
        const mode: AudioMode =
            $(element).attr('data-audio')?.toLowerCase() === 'eng'
                ? 'dub'
                : 'sub';
