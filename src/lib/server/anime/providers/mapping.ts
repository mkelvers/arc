...
export async function providerMediaId(
    anilistId: number,
    provider: string,
) {
...
export async function saveProviderMediaId(
    anilistId: number,
    provider: string,
    id: string,
) {
    ...
    await db
        ...
        .onConflictDoUpdate({
            target: [
                animeProviderMapping.anilistId,
                animeProviderMapping.provider,
            ],
            set: {
                providerMediaId: id,
                verifiedAt: now,
...
export async function verifyProviderMediaId(
    anilistId: number,
    provider: string,
) {
    await db
