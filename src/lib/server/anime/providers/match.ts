...
export function releaseInventoryEvidence<T extends { number: number; title: string }>(
  ...
  const releaseMatches = references.filter((reference) =>
    candidates.some((candidate) => episodeTitleScore(reference.title ?? '', candidate.title) >= 60)
  ).length;
...
function releaseTitleWords(title: string) {
  return new Set(
    normalizedProviderTitle(title)
      ...
      .filter(
        (word) =>
          word.length > 1 && !/^\d+(?:st|nd|rd|th)?$/.test(word) && !releaseTitleStopWords.has(word)
      )
  );
}
...
export function matchProviderStreamEpisode<T extends { number: number; title: string }>(
  ...
  const evidence = releaseInventoryEvidence(episodes, reference.release, reference.relatedReleases);
  ...
  if (Number.isInteger(reference.number) && reference.number > 0 && episodes.length > 1) {
    const numbered = episodes.find((episode) => episode.number === reference.number);
    if (numbered) {
      return numbered;
    }
