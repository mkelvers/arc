...
function child(data: Uint8Array, parent: Box, type: string) {
  return boxes(data, parent.contentStart, parent.end).find((box) => box.type === type);
}
...
function trackStart(data: Uint8Array, trak: Box, timescale: number) {
  ...
  for (let index = 0; index < entryCount; index++) {
    ...
    const segmentDuration =
      version === 1 ? Number(view.getBigUint64(offset)) : view.getUint32(offset);
    const mediaTime =
      version === 1 ? view.getBigInt64(offset + 8) : BigInt(view.getInt32(offset + 4));
...
export function audioDelayFromMp4(data: Uint8Array) {
  const moov = boxes(data, 0, data.byteLength).find((box) => box.type === 'moov');
  if (!moov) {
    return 0;
  }
