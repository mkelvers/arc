...
const starts = new RequestCache<string, AnimeSeasonStartYears>(
    24 * 60 * 60 * 1_000,
);

function startYear(
    media: Array<{ seasonYear: number | null } | null> | null | undefined,
) {
...
async function requestSeasonStarts() {
    ...
    return Object.fromEntries(
        entries.filter(
            (entry): entry is [AnimeSeason, number] =>
                entry[1] !== undefined,
        ),
    );
}
    ...
    return Effect.tryPromise({
        try: load,
            ...
            cause instanceof GraphQLRequestError
                ? cause
                : new GraphQLRequestError({ message, cause }),
    });
}
...
export function getSimulcastSeasonStarts() {
    return requestEffect(
        cachedSeasonStarts,
        'Simulcast seasons could not be loaded',
    );
}
