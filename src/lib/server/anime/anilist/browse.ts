...
export interface AniListBrowseFilters
    extends Omit<BrowseFilters, 'format' | 'status'> {
...
export function browseMediaSort(
    filters: Pick<BrowseFilters, 'sort' | 'order'>,
): MediaSort {
    const sort = filters.sort === 'score' ? 'SCORE' : 'POPULARITY';
    return filters.order === 'desc' ? `${sort}_DESC` : sort;
}

async function requestBrowsePage(
...
async function requestBrowseTaxonomy() {
    const response = await Effect.runPromise(
        request(BrowseAnimeTaxonomyDocument, {}),
    );
