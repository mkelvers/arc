...
describe('PostgreSQL episode refresh lifecycle', () => {
    databaseTest('deduplicates refresh targets in one schedule', async () => {
        await withQueue(async ({ queue, sql }) => {
            await queue.schedule([
                { anilistId: 1, targetEpisode: 2, runAt: new Date('2026-08-17T12:00:00Z') },
                { anilistId: 1, targetEpisode: 2, runAt: new Date('2026-08-17T11:00:00Z') },
            ]);
