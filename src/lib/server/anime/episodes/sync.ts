...
async function fetchAndStore(anime: AniListAnime, metadataSource: StoredMapping | undefined) {
  ...
  const [storedText, previousMetadataExternalIdId] = await Promise.all([
    db
      ...
      .then((rows) => new Map(rows.map(({ episodeId, ...text }) => [episodeId, text] as const))),
    db
      .select({
        metadataExternalIdId: animeEpisodeSync.metadataExternalIdId,
      })
  ...
  await db.transaction(async (tx) => {
    ...
    const values = source.map((episode) => {
      ...
      return {
        ...
        metadataTitleSource: media?.titleSource ?? previousMetadata?.metadataTitleSource ?? null,
    ...
    await tx
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          metadataTitleSource: sql.raw(`excluded.${animeEpisode.metadataTitleSource.name}`),
...
export async function refreshEpisodes(anime: AniListAnime, metadataSource?: StoredMapping) {
  ...
  const request = fetchAndStore(anime, metadataSource).catch(async (cause) => {
    await recordFailure(anime.id, cause).catch((failure) =>
      console.error(`Could not record episode refresh failure for AniList ${anime.id}`, failure)
    );
    throw cause;
  });
