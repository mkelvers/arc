...
describe('AniList release episode boundaries', () => {
    test('removes a later part combined into the first part inventory', () => {
        ...
        const dates = [
            ...Array.from(
                { length: 12 },
                (_, index) =>
                    new Date(Date.UTC(2018, 6, 23 + index * 7))
                        .toISOString()
                        .slice(0, 10),
            ),
            ...Array.from(
                { length: 10 },
                (_, index) =>
                    new Date(Date.UTC(2019, 3, 29 + index * 7))
                        .toISOString()
                        .slice(0, 10),
            ),
        ];
    ...
    test('keeps in-window recap specials in addition to regular episodes', () => {
        ...
        const dates = episodes.map((_, index) =>
            new Date(Date.UTC(2024, 2, 30 + index * 7))
                .toISOString()
                .slice(0, 10),
        );
    ...
    test('drops an unmatched provider pre-roll when regular episodes match', () => {
        ...
        const matched = metadata(
            ...
            Array.from(
                { length: 12 },
                (_, index) =>
                    new Date(Date.UTC(2025, 0, 1 + index * 7))
                        .toISOString()
                        .slice(0, 10),
            ),
        );
    ...
    test('drops unconfirmed later-provider inventory candidates', () => {
        ...
        const supplemental = source(
            Array.from({ length: 12 }, (_, index) => index + 14),
        ).map((episode) => ({
            ...episode,
            supplemental: true,
        }));
        ...
        const matched = metadata(
            ...
            Array.from(
                { length: 13 },
                (_, index) =>
                    new Date(Date.UTC(2020, 6, 9 + index * 7))
                        .toISOString()
                        .slice(0, 10),
            ),
        );
