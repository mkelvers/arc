...
export function episodesForRelease(
    ...
    const confirmed = selected.filter(
        (episode) =>
            !episode.supplemental || metadata.has(episode.id),
    );

    if (
        confirmed.length >= expected &&
    ...
    if (start !== null || end !== null) {
        const inReleaseWindow = selected.filter((episode) => {
            ...
            return !(
                (start !== null &&
                    releasedAt < start - releaseDateGrace) ||
                (end !== null &&
                    releasedAt > end + releaseDateGrace)
            );
        });

        if (
            inReleaseWindow.length >= expected &&
            inReleaseWindow.length < selected.length
        ) {
    ...
    if (selected.length > expected) {
        ...
        if (unmatchedSpecials.length === excess) {
            const remove = new Set(
                unmatchedSpecials.map(({ id }) => id),
            );
            selected = selected.filter(
                (episode) => !remove.has(episode.id),
            );
        }
    }
