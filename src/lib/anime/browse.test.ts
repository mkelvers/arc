...
describe('browse filters', () => {
    ...
    test('round trips non-default filters', () => {
        ...
        expect(parseBrowseFilters(new URL(href, 'https://arc.test').searchParams)).toEqual(
            filters,
        );
    });
    ...
    test.each([
        ...
        'sort=unknown',
        'order=unknown',
    ])('rejects invalid filter values in %s', (query) => {
    ...
    test.each([
        ...
        'rejects malformed metadata filters in %s',
        (query) => {
            expect(parseBrowseFilters(new URLSearchParams(query))).toBeNull();
        },
    );
