<script lang="ts">
  ...
  const backgroundLabel = $derived(
    backgroundOptions.find((option) => option.value === subtitleBackground)?.label ?? 'None'
  );
...
>
    ...
    >
      ...
      <span
        ...
        class={cn(
          ...
          autoplay ? 'border-player-accent bg-player-accent/20' : 'border-white/55 bg-white/12'
        )}
      >
        <span
          class={cn(
  ...
  {:else}
    ...
    {@const subtitleOption =
      view === 'subtitle-size' ? 'Size' : view === 'subtitle-background' ? 'Background' : null}
    <button
      type="button"
      role="menuitem"
      ...
      onclick={() => (view = editingKind ? 'segments' : subtitleOption ? 'subtitles' : 'main')}
    >
