<script lang="ts">
  ...
  async function persistSkipTimes(times: Pick<EpisodeSkipTimes, 'opening' | 'ending'>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          response.status === 401 ? 'Sign in to edit segments.' : 'Segments could not be saved.'
        );
      }
          ...
          >
            <div class="relative h-28 w-8 py-1.5">
              ...
              <input
                ...
                oninput={(event) => media.setVolume(Number(event.currentTarget.value))}
              />
            </div>
          </div>
        ...
        <div class="relative">
          ...
          {#if player.settingsOpen}
            <Settings
              ...
              canEditSkipTimes={canEditSkipTimes}
              mode={media.mode}
